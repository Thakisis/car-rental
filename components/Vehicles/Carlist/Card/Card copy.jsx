...
export default function Card(props) {
    ...
    const IconSidebar = useMemo(() => IconSide.map(({ icon, value, props }) => {
        ...
        //console.log(props)
        return (<div key={value} className={`flex-1 p-2 pl-2 transition-all	${selected === value ? "bg-slate-300" : "bg-slate-200"} `} onClick={() => setSelected(value)} ><Icono selected={selected === value} {...props} className="w-6 h-6 text-black transition-all	" /></div>)
    ...
    const Sections = [<MainContent {...MainProps} key="maincontent" />, <VehicleContent {...VehiculoProps} key="vehiclecontent" />, <EngineContent {...MotorProps} key="enginaContent" />]
    ...
    return (
        ...
        <div className="lg:h-80 mx-auto flex flex-wrap bg-slate-200 rounded-lg rounded-bl-none lg:rounded-lg lg:rounded-tr-none  shadow-2xl relative mb-9 lg:mb-0">
            <div className="relative lg:h-80  rounded-lg rounded-b-none lg:rounded-none lg:rounded-l-lg  overflow-hidden  ">
                ...
                <img src={`/images/card/${image}`} alt={nombre} class=" h-full w-auto" />
            </div>
            <div
